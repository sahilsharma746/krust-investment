...
// Find the max and min y values from your dataset
    const allYValues = originalData.flatMap(candle => candle.y);
    const maxY = Math.max(...allYValues);
    const minY = Math.min(...allYValues);

    // Transform data to reverse the y-axis
    const reversedData = originalData.map(candle => {
      const [open, high, low, close] = candle.y;
      return {
        x: candle.x,
        y: [
          maxY - (open - minY),  // Reversed open
          maxY - (low - minY),   // Reversed high becomes low
          maxY - (high - minY),  // Reversed low becomes high
          maxY - (close - minY)  // Reversed close
        ]
      };
    });
